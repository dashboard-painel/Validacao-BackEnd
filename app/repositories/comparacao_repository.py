"""Repository façade — roteia chamadas para clients e local_db."""
...
from app.clients.coletor_bi import buscar_por_associacao as _buscar_por_associacao
...
def buscar_status_farmacias(codigos: list[str], executor: ThreadPoolExecutor | None = None) -> dict[str, str]:
    return _buscar_status_farmacias(codigos, executor=executor)


def buscar_por_associacao(codigos: list[str], executor: ThreadPoolExecutor | None = None) -> dict[str, dict]:
    return _buscar_por_associacao(codigos, executor=executor)
...
def salvar_status_farmacias(comparacao_id: int, associacao: str, status_dict: dict[str, str], coletor_bi: dict[str, dict] | None = None, classificacao_dict: dict[str, str | None] | None = None) -> None:
...
def buscar_classificacao_farmacias(codigos: list[str], executor: ThreadPoolExecutor | None = None) -> dict[str, str | None]:
    return _buscar_classificacao_farmacias(codigos, executor=executor)


def buscar_versoes_farmacias(codigos: list[str], executor: ThreadPoolExecutor | None = None) -> dict[str, str | None]:
    return _buscar_versoes_farmacias(codigos, executor=executor)